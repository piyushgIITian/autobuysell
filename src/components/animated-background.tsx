...
import { Feature, FeatureNames, FEATURES } from "@/data/constants";
...
const AnimatedBackground = () => {
  ...
  const handleMouseHover = (e: SplineEvent) => {
    if (!splineApp || selectedFeature?.name === e.target.name) return;
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      ...
      if (!selectedFeature || selectedFeature.name !== e.target.name) {
        const feature = FEATURES[e.target.name as FeatureNames];
  ...
  useEffect(() => {
    if (!selectedFeature || !splineApp) return;
    splineApp.setVariable("heading", selectedFeature.label);
    splineApp.setVariable("desc", selectedFeature.shortDescription);
  }, [selectedFeature]);
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyDown", (e) => {
      ...
      splineApp.setVariable("heading", feature.label);
      splineApp.setVariable("desc", feature.shortDescription);
    });
  ...
  const getKeycapsAnimation = () => {
    ...
    const stop = () => {
      ...
      Object.values(FEATURES).forEach((feature) => {
        const keycap = splineApp.findObjectByName(feature.name);
        if (!keycap) return;
        const t = gsap.to(keycap?.position, {
          y: 0,
          duration: 4,
    ...
    <>
      <Suspense fallback={<div>Loading...</div>}>
        <Spline
          ...
          scene="/assets/untitled_3.spline"
        />
      </Suspense>
    </>
  );
